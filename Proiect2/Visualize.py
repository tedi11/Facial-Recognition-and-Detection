...
def show_detections_with_ground_truth(detections, scores, recognitions, confidence, file_names, params: Parameters):
    """
    ...
    for test_file in test_files:
        ...
        indices_detections_current_image = np.where(file_names == short_file_name)
        current_detections = detections[indices_detections_current_image]
        current_recognitions = recognitions[indices_detections_current_image]
        current_confidence = confidence[indices_detections_current_image]
        current_scores = scores[indices_detections_current_image]

        for idx, (detection, recognition, confidence2) in enumerate(zip(current_detections, current_recognitions, current_confidence)):
            if recognition == 0:
                print("dad")
                character = "Dad"
            elif recognition == 1:
                ...
                character = "Other"
            cv.rectangle(image, (detection[0], detection[1]), (detection[2], detection[3]), (0, 0, 255), thickness=1)
            cv.putText(image, f"{character} confidence: {confidence2} score:" + str(current_scores[idx])[:4], (detection[0], detection[1]),
                       cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
